const svc = require('./leases.service');
...
const terminate = async (req, res, next) => {
  try { await svc.terminate(req.params.id, req.orgFilter); res.json({ success: true, message: 'Lease terminated.' }); } catch (e) { next(e); }
};
const remove = async (req, res, next) => {
  try { await svc.remove(req.params.id, req.orgFilter); res.json({ success: true, message: 'Lease deleted.' }); } catch (e) { next(e); }
};
